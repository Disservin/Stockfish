...
void MainThread::search()
{
    ...
    Search::Skill skill =
        Search::Skill(Options["Skill Level"], Options["UCI_LimitStrength"] ? int(Options["UCI_Elo"]) : 0);

    if (int(Options["MultiPV"]) == 1 && !Search::Limits.depth && !skill.enabled() && rootMoves[0].pv[0] != MOVE_NONE)
...
void MainThread::check_time()
{
    ...
    callsCnt = Search::Limits.nodes ? std::min(1024, int(Search::Limits.nodes / 1024)) : 1024;
    ...
    if ((Search::Limits.use_time_management() && (elapsed > Time.maximum() - 10 || stopOnPonderhit)) ||
...
void Thread::search()
{
    ...
    Search::Skill skill(Options["Skill Level"], Options["UCI_LimitStrength"] ? int(Options["UCI_Elo"]) : 0);
    ...
    while (++rootDepth < MAX_PLY && !Threads.stop &&
           !(Search::Limits.depth && mainThread && rootDepth > Search::Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                bestValue = Stockfish::Search::search<Search::Root>(rootPos, ss, alpha, beta, adjustedDepth, false);
        ...
        if (Search::Limits.use_time_management() && !Threads.stop && !mainThread->stopOnPonderhit)
        {
    ...
    if (skill.enabled())
        std::swap(rootMoves[0],
                  *std::find(rootMoves.begin(), rootMoves.end(), skill.best ? skill.best : skill.pick_best(multiPV)));
}
...
template <NodeType nodeType> Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode)
{
    ...
    if (!PvNode && ss->ttHit && tte->depth() > depth - (tte->bound() == BOUND_EXACT) &&
    ...
    if (!rootNode && TB::Cardinality)
    {
        ...
        if (piecesCount <= TB::Cardinality && (piecesCount < TB::Cardinality || depth >= TB::ProbeDepth) &&
            pos.rule50_count() == 0 && !pos.can_castle(ANY_CASTLING))
        {
            ...
            if (err != TB::ProbeState::FAIL)
            {
                ...
                {
                    tte->save(posKey, value_to_tt(value, ss->ply), ss->ttPv, b, std::min(MAX_PLY - 1, depth + 6),
    ...
    if (!ss->ttPv && depth < 8 && eval - futility_margin(depth, improving) - (ss - 1)->statScore / 303 >= beta &&
        eval >= beta && eval < 28031) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~22 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17139 && eval >= beta &&
        eval >= ss->staticEval && ss->staticEval >= beta - 20 * depth - improvement / 13 + 233 + complexity / 25 &&
        !excludedMove && pos.non_pawn_material(us) && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        assert(eval - beta >= 0);
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (move != excludedMove && pos.legal(move))
            {
                ...
                ss->continuationHistory =
                    &thisThread->continuationHistory[ss->inCheck][true][pos.moved_piece(move)][to_sq(move)];
...
moves_loop: // When in check, search starts here
    ...
    if (ss->inCheck && !PvNode && depth >= 2 && ttCapture && (tte->bound() & BOUND_LOWER) &&
        tte->depth() >= depth - 3 && ttValue >= probCutBeta && abs(ttValue) <= VALUE_KNOWN_WIN &&
        abs(beta) <= VALUE_KNOWN_WIN)
        return probCutBeta;

    const PieceToHistory *contHist[] = {
        (ss - 1)->continuationHistory, (ss - 2)->continuationHistory, nullptr, (ss - 4)->continuationHistory, nullptr,
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (rootNode && thisThread == Threads.main() && Time.elapsed() > 3000)
            sync_cout << "info depth " << depth << " currmove " << UCI::move(move, pos.is_chess960())
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && !PvNode && lmrDepth < 7 && !ss->inCheck &&
                    ss->staticEval + 180 + 201 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))] +
                            captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] / 6 <
                        alpha)
                    continue;
            ...
            {
                int history = (*contHist[0])[movedPiece][to_sq(move)] + (*contHist[1])[movedPiece][to_sq(move)] +
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && depth >= 4 - (thisThread->previousDepth > 24) + 2 * (PvNode && tte->is_pv()) &&
                move == ttMove &&
        ...
        ss->currentMove = move;
        ss->continuationHistory = &thisThread->continuationHistory[ss->inCheck][capture][movedPiece][to_sq(move)];
            ...
            (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (depth > 9 && (mp.threatenedPieces & from_sq(move)))
                r--;
            ...
            ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)] + (*contHist[0])[movedPiece][to_sq(move)] +
                            (*contHist[1])[movedPiece][to_sq(move)] + (*contHist[3])[movedPiece][to_sq(move)] - 4433;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                if (PvNode && value < beta) // Update alpha! Always alpha < beta
                {
                    ...
                    if (depth > 1 && depth < 6 && beta < VALUE_KNOWN_WIN && alpha > -VALUE_KNOWN_WIN)
                        depth -= 1;
...
// (~155 elo)
template <NodeType nodeType> Value qsearch(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth)
{
    ...
    if (!PvNode && ss->ttHit && tte->depth() >= ttDepth && ttValue != VALUE_NONE // Only in case of TT access race
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
        return ttValue;
    ...
    else
    {
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                tte->save(posKey, value_to_tt(bestValue, ss->ply), false, BOUND_LOWER, DEPTH_NONE, MOVE_NONE,
                          ss->staticEval);
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && !givesCheck && to_sq(move) != prevSq &&
        ...
        ss->currentMove = move;
        ss->continuationHistory =
            &thisThread->continuationHistory[ss->inCheck][capture][pos.moved_piece(move)][to_sq(move)];
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && quietCheckEvasions > 1 && !capture && ss->inCheck)
            continue;
...
Move Skill::pick_best(size_t multiPV)
{
    ...
    for (size_t i = 0; i < multiPV; ++i)
    {
        ...
        int push =
            int((weakness * int(topScore - rootMoves[i].score) + delta * (rng.rand<unsigned>() % int(weakness))) / 128);
...
template <bool isRoot> uint64_t perft(Position &pos, Depth depth)
{
...
string UCI::pv(const Position &pos, Depth depth, Value alpha, Value beta)
{
    ...
    for (size_t i = 0; i < multiPV; ++i)
    {
        ...
        ss << "info"
           << " depth " << d << " seldepth " << rootMoves[i].selDepth << " multipv " << i + 1 << " score "
           << UCI::value(v);
