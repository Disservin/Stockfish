...
void MainThread::search() {
  if (Search::Limits.perft) {
    nodes = Search::perft<true>(rootPos, Search::Limits.perft);
    sync_cout << "\nNodes searched: " << nodes << "\n" << sync_endl;
    return;
  }

  Color us = rootPos.side_to_move();
  Time.init(Search::Limits, us, rootPos.game_ply());
  TT.new_search();
  ...
  while (!Threads.stop && (ponder || Search::Limits.infinite)) {
  ...
  if (Search::Limits.npmsec)
    Time.availableNodes += Search::Limits.inc[us] - Threads.nodes_searched();

  Thread *bestThread = this;
  Search::Skill skill =
      Search::Skill(Options["Skill Level"],
                    Options["UCI_LimitStrength"] ? int(Options["UCI_Elo"]) : 0);

  if (int(Options["MultiPV"]) == 1 && !Search::Limits.depth &&
      !skill.enabled() && rootMoves[0].pv[0] != MOVE_NONE)
    bestThread = Threads.get_best_thread();

  bestPreviousScore = bestThread->rootMoves[0].score;
  bestPreviousAverageScore = bestThread->rootMoves[0].averageScore;

  for (Thread *th : Threads) th->previousDepth = bestThread->completedDepth;
...
void Thread::search() {
  ...
  std::memset(ss - 7, 0, 10 * sizeof(Search::Stack));
  for (int i = 7; i > 0; i--)
  ...
  Search::Skill skill(Options["Skill Level"], Options["UCI_LimitStrength"]
                                                  ? int(Options["UCI_Elo"])
                                                  : 0);
  ...
  while (++rootDepth < MAX_PLY && !Threads.stop &&
         !(Search::Limits.depth && mainThread &&
           rootDepth > Search::Limits.depth)) {
    // Age out PV variability metric
    if (mainThread) totBestMoveChanges /= 2;

    // Save the last iteration's scores before first PV line is searched and
    // all the move scores except the (new) PV are set to -VALUE_INFINITE.
    for (Search::RootMove &rm : rootMoves) rm.previousScore = rm.score;
    ...
    for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx) {
      ...
      int failedHighCnt = 0;
      while (true) {
        // Adjust the effective depth searched, but ensuring at least one
        // effective increment for every four searchAgain steps (see issue
        // #2717).
        Depth adjustedDepth = std::max(
            1, rootDepth - failedHighCnt - 3 * (searchAgainCounter + 1) / 4);
        bestValue = Stockfish::Search::search<Search::Root>(
            rootPos, ss, alpha, beta, adjustedDepth, false);
    ...
    if (Search::Limits.mate && bestValue >= VALUE_MATE_IN_MAX_PLY &&
        VALUE_MATE - bestValue <= 2 * Search::Limits.mate)
      Threads.stop = true;
    ...
    if (Search::Limits.use_time_management() && !Threads.stop &&
        !mainThread->stopOnPonderhit) {
      double fallingEval =
          (71 + 12 * (mainThread->bestPreviousAverageScore - bestValue) +
      ...
      double reduction =
          (1.4 + mainThread->previousTimeReduction) / (2.15 * timeReduction);
      double bestMoveInstability =
          1 + 1.7 * totBestMoveChanges / Threads.size();
...
Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth,
  ...
  if (!excludedMove) ss->ttPv = PvNode || (ss->ttHit && tte->is_pv());
  ...
  if (ss->inCheck) {
    ...
    else  // Fall back to (semi)classical complexity for TT hits, the NNUE
          // complexity is lost
  ...
  if (is_ok((ss - 1)->currentMove) && !(ss - 1)->inCheck && !priorCapture) {
    int bonus = std::clamp(-19 * int((ss - 1)->staticEval + ss->staticEval),
                           -1914, 1914);
    thisThread->mainHistory[~us][from_to((ss - 1)->currentMove)] << bonus;
  }

  // Set up the improvement variable, which is the difference between the
  // current static evaluation and the previous static evaluation at our turn
  // (if we were in check at our previous move we look at the move prior to it).
  // The improvement margin and the improving flag are used in various pruning
  // heuristics.
  ...
  if (!ss->ttPv && depth < 8 &&
      eval - futility_margin(depth, improving) - (ss - 1)->statScore / 303 >=
          beta &&
      eval >= beta &&
  ...
  if (!PvNode && (ss - 1)->currentMove != MOVE_NULL &&
      (ss - 1)->statScore < 17139 && eval >= beta && eval >= ss->staticEval &&
      ss->staticEval >=
          beta - 20 * depth - improvement / 13 + 233 + complexity / 25 &&
      !excludedMove && pos.non_pawn_material(us) &&
      (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor)) {
    assert(eval - beta >= 0);

    // Null move dynamic reduction based on depth, eval and complexity of
    // position
    Depth R = std::min(int(eval - beta) / 168, 7) + depth / 3 + 4 -
              (complexity > 861);
  ...
  if (!PvNode && depth > 4 &&
    ...
    MovePicker mp(pos, ttMove, probCutBeta - ss->staticEval, depth - 3,
                  &captureHistory);
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
      if (move != excludedMove && pos.legal(move)) {
        ...
        ss->currentMove = move;
        ss->continuationHistory =
            &thisThread->continuationHistory[ss->inCheck][true][pos.moved_piece(
                move)][to_sq(move)];
...
moves_loop:  // When in check, search starts here
  ...
  if (ss->inCheck && !PvNode && depth >= 2 && ttCapture &&
  ...
  while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
    ...
    // Step 14. Pruning at shallow depth (~98 Elo). Depth conditions are
    // important for mate finding.
    if (!rootNode && pos.non_pawn_material(us) &&
      ...
      int lmrDepth =
          std::max(newDepth - reduction(improving, depth, moveCount, delta,
                                        thisThread->rootDelta),
                   0);

      if (capture || givesCheck) {
        // Futility pruning for captures (~0 Elo)
        if (!givesCheck && !PvNode && lmrDepth < 7 && !ss->inCheck &&
            ss->staticEval + 180 + 201 * lmrDepth +
        ...
        if (!pos.see_ge(move, Value(-222) * depth)) continue;
      } else {
    ...
    if (ss->ply < thisThread->rootDepth * 2) {
      ...
      // ttMove.
      if (!rootNode &&
          depth >= 4 - (thisThread->previousDepth > 24) +
                       2 * (PvNode && tte->is_pv()) &&
          move == ttMove &&
        ...
        // reduced search without the ttMove. So we assume this expected
        // Cut-node is not singular, that multiple moves fail high, and we can
        // prune the whole subtree by returning a soft bound.
        else if (singularBeta >= beta)
          return singularBeta;

        // If the eval of ttMove is greater than beta, we reduce it (negative
        // extension)
        else if (ttValue >= beta)
          extension = -2;

        // If the eval of ttMove is less than alpha and value, we reduce it
        // (negative extension)
    ...
    ss->currentMove = move;
    ss->continuationHistory =
        &thisThread->continuationHistory[ss->inCheck][capture][movedPiece]
                                        [to_sq(move)];
    ...
    if (depth >= 2 && moveCount > 1 + (PvNode && ss->ply <= 1) &&
      ...
      Depth r =
          reduction(improving, depth, moveCount, delta, thisThread->rootDelta);
      ...
      if (depth > 9 && (mp.threatenedPieces & from_sq(move))) r--;
      ...
      ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)] +
                      ...
                      (*contHist[1])[movedPiece][to_sq(move)] +
                      (*contHist[3])[movedPiece][to_sq(move)] - 4433;
      ...
      if (value > alpha && d < newDepth) {
        ...
        value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                               newDepth + doDeeperSearch, !cutNode);
    ...
    if (rootNode) {
      ...
      if (moveCount == 1 || value > alpha) {
        ...
        for (Move *m = (ss + 1)->pv; *m != MOVE_NONE; ++m) rm.pv.push_back(*m);

        // We record how often the best move has been changed in each iteration.
        // This information is used for time management. In MultiPV mode,
        // we must take care to only do this for the first PV line.
        if (moveCount > 1 && !thisThread->pvIdx) ++thisThread->bestMoveChanges;
      } else
    ...
    if (value > bestValue) {
      ...
      if (value > alpha) {
        ...
        if (PvNode && value < beta)  // Update alpha! Always alpha < beta
        {
          ...
          if (depth > 1 && depth < 6 && beta < VALUE_KNOWN_WIN &&
...
Value qsearch(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth) {
  ...
  while ((move = mp.next_move()) != MOVE_NONE) {
    ...
    if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && !givesCheck &&
        to_sq(move) != prevSq && futilityBase > -VALUE_KNOWN_WIN &&
    ...
    ss->currentMove = move;
    ss->continuationHistory =
        &thisThread->continuationHistory[ss->inCheck][capture]
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
  ...
  return (r + 1642 - int(delta) * 1024 / int(rootDelta)) / 1024 +
         (!i && r > 916);
}
...
Skill::Skill(int skill_level, int uci_elo) {
  if (uci_elo)
    level =
        std::clamp(std::pow((uci_elo - 1346.6) / 143.4, 1 / 0.806), 0.0, 20.0);
  else
...
void MainThread::check_time() {
  ...
  callsCnt = Search::Limits.nodes
                 ? std::min(1024, int(Search::Limits.nodes / 1024))
                 : 1024;
  ...
  if ((Search::Limits.use_time_management() &&
      ...
      (Search::Limits.nodes &&
       Threads.nodes_searched() >= (uint64_t)Search::Limits.nodes))
    Threads.stop = true;
}
...
string UCI::pv(const Position &pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i) {
    ...
    ss << " nodes " << nodesSearched << " nps "
       << nodesSearched * 1000 / elapsed << " hashfull " << TT.hashfull()
       << " tbhits " << tbHits << " time " << elapsed << " pv";
