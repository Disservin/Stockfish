           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                bestValue = is_mainthread()
                            ? search<true, Root>(rootPos, ss, alpha, beta, adjustedDepth, false)
                            : search<false, Root>(rootPos, ss, alpha, beta, adjustedDepth, false);
...
template<bool mainThread, NodeType nodeType>
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && ss->ply >= thisThread->nmpMinPly && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Value nullValue =
          -search<mainThread, NonPV>(pos, ss + 1, -beta, -beta + 1, depth - R, false);
        ...
        if (nullValue >= beta && nullValue < VALUE_TB_WIN_IN_MAX_PLY)
        {
            ...
            Value v = search<mainThread, NonPV>(pos, ss, beta - 1, beta, depth - R, false);
        ...
        && !(ttData.depth >= depth - 3 && ttData.value != VALUE_NONE && ttData.value < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            if (value >= probCutBeta)
                value = -search<mainThread, NonPV>(pos, ss + 1, -probCutBeta, -probCutBeta + 1,
                                                   depth - 4, !cutNode);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (rootNode && mainThread && nodes > 10000000)
        {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && ttData.depth >= depth - 3)
            {
                ...
                value = search<mainThread, NonPV>(pos, ss, singularBeta - 1, singularBeta,
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            value = -search<mainThread, NonPV>(pos, ss + 1, -(alpha + 1), -alpha, d, true);
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                if (newDepth > d)
                    value = -search<mainThread, NonPV>(pos, ss + 1, -(alpha + 1), -alpha, newDepth,
                                                       !cutNode);
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<mainThread, NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
        ...
        if (PvNode && (moveCount == 1 || value > alpha))
        {
            ...
            value = -search<mainThread, PV>(pos, ss + 1, -beta, -alpha, newDepth, false);
        }
