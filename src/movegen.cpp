...
splat_precomputed_moves(Move* moveList, Square from, Bitboard occupied, Bitboard target) {
    ...
    // For sliders, derive it from the actual attack bitboard so this path also works with
    // hyperbola attack generation.
    uint32_t mask;
    if constexpr (Pt == BISHOP || Pt == ROOK)
    {
        mask = pext(attacks_bb<Pt>(from, occupied) & target, PseudoAttacks[Pt][from]);
