    ...
    static void transform_perspective(const std::array<i16, HalfDimensions>&      accumulation,
        ...
        for (IndexType j = 0; j < NumOutputChunks; j += 2)
        {
            ...
            for (IndexType k = 0; k < 2; ++k)
            {
                ...
                const vec_t clipped0 = vec_packus_16(acc0a, acc0b);
                const vec_t sum0a    = vec_srli_16(vec_unpacklo_8(Zero, clipped0), 1);
                const vec_t sum0b    = vec_srli_16(vec_unpackhi_8(Zero, clipped0), 1);
        #else
